...
def get_user_data(cursor, user):
    ...
    # add placas
    cursor.execute(f"SELECT * FROM placas WHERE IdMember_FK = {member}")
    _info.update({"placas": [i[2] for i in cursor.fetchall()]})
        ...
        _info.update(
            {
                "brevete": {
                    ...
                    "puntos": _m[8],
                    "record": _m[9],
                    "vigencia": f"{_vigencia:,} días" if _vigencia > 0 else "Vencida",
                }
            }
        )
    else:
    ...
    for satimp in cursor.fetchall():
        cursor.execute(f"SELECT * FROM satimpDeudas WHERE IdCodigo_FK = {satimp[0]}")
        _s = []
        for _x in cursor.fetchall():
            _s.append(
                {
                    "ano": _x[1],
                    "periodo": _x[2],
    ...
    _images = []
    cursor.execute(
        f"""SELECT * FROM sunarps WHERE IdPlaca_FK IN (SELECT IdPlaca FROM placas WHERE IdMember_FK = {member})
            ORDER BY LastUpdate DESC"""
    )
    for _m in cursor.fetchall():
        # add image to attachment list
        _img_path = os.path.abspath(os.path.join(".", "static", "images", _m[15]))
        if os.path.isfile(_img_path):
            _images.append(str(os.path.basename(_img_path)))
