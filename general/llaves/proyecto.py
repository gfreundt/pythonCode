...
class Proyecto:
    ...
    def __init__(self, libro_ggmk):
        self.libro_ggmk = libro_ggmk
        self.window2 = Tk()
    ...
    def nuevo_proyecto(self):
        ...
        self.boton1 = Button(self.window2, text="Agrega GMK", command=self.agrega_gmk)
        self.boton1.place(x=700, y=70)

        self.boton2 = Button(self.window2, text="Agrega MK", command=self.agrega_mk)
        self.boton2.place(x=730, y=100)
        self.boton2.config(state="disabled")

        self.boton3 = Button(
            self.window2, text="Nivel Completo", command=self.nivel_completo
        )
        self.boton3.place(x=860, y=85)

        self.boton4 = Button(self.window2, text="Deshacer", command=self.deshacer)
    ...
    def nivel_completo(self):
        if self.nivel_boton == 0:
            self.ggmk = self.asigna_llaves()
            self.window2.destroy()
            self.vista = Visor(configuracion="proyecto")
    ...
    def asigna_llaves(self):
        _proyecto = copy(self.libro_ggmk)

        # ajusta numero de GMKs
        _proyecto["subkeys"] = _proyecto["subkeys"][: len(self.arbol)]

        # ajusta numero de MKs
        for g, gmk in zip(self.arbol, _proyecto["subkeys"]):
            gmk["subkeys"] = gmk["subkeys"][: len(g)]

            # ajusta numero de Ks
            for m, mk in zip(g, gmk["subkeys"]):
                mk["subkeys"] = mk["subkeys"][:m]
