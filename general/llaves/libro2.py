...
class Libro:
    ...
    def valida_libro_completo(self):
        ...
        # eliminar llaves cruzadas (una MK de otro grupo abre el cilindro)
        c = 0
        for gmk in self.ggmk["subkeys"]:
            for mk in gmk["subkeys"]:
                for k_sec, k in enumerate(mk["subkeys"]):
                    valida = self.valida_llave_no_cruzada(
                        k["cilindro"],
                        [self.ggmk["codigo"], gmk["codigo"], mk["codigo"]],
                    )
    ...
    def genera_mp_pos(self):
        ...
        # formato 1-1-2-3
        self.mp_pos = {"gmk": _pines[0:1], "mk": _pines[1:3], "k": _pines[3:]}
