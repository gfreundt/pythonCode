    ...
    def monitor_without_threads(self):
        ...
        while True:
            ...
            if self.last_change + td(seconds=120) < dt.now():
                self.stalled = True
            # build and output status string
            try:
                ...
                _captcha_rate = 1
            status = f"[{dt.now().strftime('%H:%M:%S')}] Processed: {self.progress}\n"
    ...
    def dashboard(self):
        ...
        for record in self.database:
            ...
            if dt.strptime(
                ...
                rtec_actualizado += 1
        print(
            f"Tienen Documento: {with_documento} ({with_documento*100/self.len_database:.2f}%)"
        )
        print(
            f"Tienen Brevete: {with_brevete} ({with_brevete*100/self.len_database:.2f}%)"
        )
        print(f"Tienen RTec: {with_rtec} ({with_rtec*100/self.len_database:.2f}%)")
        print(
            f"Tienen Documento: {with_documento} ({with_documento*100/self.len_database:.2f}%)"
        )
...
class Brevete:
    ...
    def full_update(self, start_record, end_record):
        ...
        while MONITOR.stalled:
            ...
            MONITOR.progress = 0
            # MONITOR.start_time = dt.now() - td(seconds=1)
