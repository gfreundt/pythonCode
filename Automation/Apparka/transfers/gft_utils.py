...
class GoogleUtils:
    ...
    def get_drive_files(self, gdrive_path_id):
        file_list = self.gdrive.ListFile({'q': f"'{gdrive_path_id}' in parents"}).GetList()
        return file_list
    
    def download_from_drive(self, gdrive_object, local_path):
        gdrive_object.GetContentFile(local_path)

    def upload_to_drive(self, local_path, gdrive_filename, gdrive_path_id):
        # select file to upload
        file = self.gdrive.CreateFile({"title": gdrive_filename, "parents":[{"id":gdrive_path_id}]})
