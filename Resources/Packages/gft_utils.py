...
class WhatsAppUtils:
    ...
    def send(self, celnum, message):
        ...
        # wait until Whatsapp has finished loading, if exceeds timeout return error code
        timeout = 0
        xpath = []
        while len(xpath) < 1:
            xpath = webdriver.find_elements(
                By.XPATH,
                "/html/body/div[1]/div/div/div[2]/div[3]/header/header/div/span/div/span/div[1]/div",
            )
            time.sleep(1)
            timeout += 1
        ...
        try:
            # wait and click on "New Message"
            time.sleep(2)
            xpath[0].click()

            # wait and write phone number
            time.sleep(1)
            xpath = webdriver.find_element(
                By.XPATH,
                "/html/body/div[1]/div/div/div[2]/div[2]/div[1]/span/div/span/div/div[1]/div[2]/div[2]/div/div/p",
            )
            xpath.send_keys(celnum)
            time.sleep(2)
            xpath.send_keys("\n")
            ...
            xpath.send_keys(message)
            time.sleep(2)
            xpath.send_keys("\n")

            # succesful message, return ok code
            return 1

        except:
            # unsuccesful message, return error code
            return 0
    ...
    def send_from_outlook(self, emails):
        ...
        for to_send_email in emails:
            try:
                ...
                message.Bcc = to_send_email.get("bcc", "")
                message.Subject = to_send_email["subject"]
                message.Body = to_send_email.get("plain_body", "")
                message.HTMLBody = to_send_email.get("html_body", "")
                for attach_filename in to_send_email["attachments"]:
                    message.Attachments.Add(attach_filename)
                # send email
                message.Send()
                # add succesful send
                success.append(True)
            except:
                # add not succesful send
                success.append(False)

        # returns list of True/False depending on email sent
        return success


class EmailUtils_Deprecated:

    def __init__(self, account):
        dotenv.load_dotenv(
            os.path.join(
                "D:\pythonCode", "Resources", "ConfigData", "environ_variables.env"
            )
        )
        ...
        for to_send_email in emails:
            ...
            try:
                ...
                with smtplib.SMTP("smtp-mail.outlook.com", port=587) as smtp:
                    ...
                    smtp.sendmail(
                        self.sender_email, to_send_email["to"], message.as_string()
                    )
                    success.append(True)
            except KeyboardInterrupt:
                success.append(False)
    ...
    def read_outlook(self, *args, **kwargs):
        return []
